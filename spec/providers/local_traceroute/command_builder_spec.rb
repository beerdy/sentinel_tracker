...
RSpec.describe SentinelTracker::Providers::LocalTraceroute::CommandBuilder do
  ...
  it "использует fallback путь, если основной недоступен" do
    allow(File).to receive(:executable?).and_return(false)
    allow(File).to receive(:executable?).with("/usr/bin/traceroute").and_return(true)

    expect(builder.call(ip: "8.8.8.8")).to eq(
      ["/usr/bin/traceroute", "-q", "1", "-m", "8", "8.8.8.8"]
    )
  end
end
