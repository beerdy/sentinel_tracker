module SentinelTracker
  ##
  # Реестр provider'ов обогащения security event.
  class SecurityEventEnrichmentRegistry
    class << self
      ##
      ...
      def normalize_provider_name(provider_name:)
        ...
        raise ArgumentError, "Security event enrichment provider name is empty" if value.empty?
      ...
      def provider_builder_for(provider_name:)
        ...
        raise ArgumentError, "Unknown security event enrichment provider: #{provider_name}"
      end
      ...
      def provider_options_for(configuration:, provider_name:)
        options = configuration.security_event_enrichment_provider_options || {}
